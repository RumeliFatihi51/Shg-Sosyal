alter table public.communities
  add column if not exists category text not null default 'Topluluk';

create index if not exists communities_category_status_idx
on public.communities(status, category, updated_at desc);

create index if not exists profiles_mobile_search_idx
on public.profiles(username, tag, first_name, last_name);
